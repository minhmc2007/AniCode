...
Wenn AniCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
AniCode Desktop startet im Hintergrund einen lokalen AniCode-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `AniCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
AniCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit AniCode ihn neu aufbauen kann.

1. Beende AniCode Desktop komplett.
...
3. Starte AniCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

AniCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Unter Windows benoetigt AniCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit AniCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

AniCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer AniCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende AniCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von AniCode Desktop):
...
Wenn du Probleme mit AniCode hast:

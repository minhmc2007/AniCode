---
title: TUI
description: Using the AniCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

AniCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running AniCode starts the TUI for the current directory.
...
When using the AniCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to AniCode. Allows you to select from available providers and add their API keys.
...
- `leader_timeout` - Controls how long AniCode waits after the leader key. Defaults to `2000`.

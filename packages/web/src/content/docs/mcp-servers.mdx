...
You can add external tools to AniCode using the _Model Context Protocol_, or MCP. AniCode supports both local and remote servers.
...
You can define MCP servers in your [AniCode Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
AniCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, AniCode will:
...
If the server requires authentication, AniCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opencode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, AniCode will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in AniCode, alongside built-in tools. So you can manage them through the AniCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect AniCode to your Sentry account.

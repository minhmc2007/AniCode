---
title: Zen
description: Lista selecionada de modelos fornecidos pelo AniCode.
---
...
O OpenCode Zen é uma lista de modelos testados e verificados fornecidos pela equipe do AniCode.

O Zen funciona como qualquer outro provedor no AniCode. Você faz login no OpenCode Zen e recebe sua chave de API. É **completamente opcional** e você não precisa usá-lo para usar o AniCode.
...
:::tip
Testamos um grupo selecionado de modelos e provedores que funcionam bem com o AniCode.
:::
...
O OpenCode Zen funciona como qualquer outro provedor no AniCode.
...
O [model id](/docs/config/#models) na sua configuração do AniCode usa o formato `opencode/<model-id>`. Por exemplo, para GPT 5.5, você usaria `opencode/gpt-5.5` na sua configuração.
...
Você pode notar [modelos de baixo custo](/docs/config/#models), como Haiku, Nano ou Flash, no seu histórico de uso. O AniCode usa esses modelos para gerar títulos de sessões.
...
- DeepSeek V4 Flash Free está disponível no AniCode por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- MiMo-V2.5 Free está disponível no AniCode por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Laguna S 2.1 Free está disponível no AniCode por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Ling-3.0-flash Free está disponível no AniCode por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- North Mini Code Free está disponível no AniCode por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Nemotron 3 Ultra Free está disponível no AniCode por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Big Pickle é um modelo stealth que está gratuito no AniCode por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
...
4. Não ter **lock-in**, permitindo que você o use com qualquer outro agente de codificação. E sempre permitir que você use qualquer outro provedor com o AniCode também.

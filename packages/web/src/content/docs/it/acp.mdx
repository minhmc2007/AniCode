---
title: Supporto ACP
description: Usa AniCode in qualunque editor compatibile con ACP.
---

AniCode supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare AniCode via ACP, configura l'editor in modo che esegua il comando `opencode acp`.

Il comando avvia AniCode come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'AniCode' nel selettore agenti di AI Chat.
...
Per usare AniCode come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare AniCode come agente ACP per la chat.
...
AniCode funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione AniCode

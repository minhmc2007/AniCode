---
title: Provider
description: Usa qualsiasi provider LLM in AniCode.
---

import config from "../../../../config.mjs"
export const console = config.console

AniCode usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di AniCode.
...
OpenCode Zen e una lista di modelli forniti dal team AniCode, testati e
verificati per funzionare bene con AniCode. [Scopri di piu](/docs/zen).
...
Funziona come qualsiasi altro provider in AniCode ed e totalmente opzionale.
...
Per usare Amazon Bedrock con AniCode:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in AniCode non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
AniCode usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare AniCode
...
Per usare Google Vertex AI con AniCode:
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di AniCode come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di AniCode elencata come sessione separata.
...
:::tip
Ollama può auto-configurarsi per AniCode. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con AniCode:
...
3. Copia la chiave API da usare in AniCode.
...
6. **Importante**: Prima di usare modelli cloud in AniCode, devi scaricare le informazioni del modello localmente:
...
OpenCode Zen è una lista di modelli testati e verificati forniti dal team AniCode. [Scopri di più](/docs/zen).
...
3. Inserisci la tua chiave API di AniCode.
...
I campi `limit` permettono a AniCode di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.

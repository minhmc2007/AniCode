---
title: IDE
description: L'estensione AniCode per VS Code, Cursor e altri IDE
---

AniCode si integra con VS Code, Cursor o qualunque IDE che supporti un terminale. Basta eseguire `opencode` nel terminale per iniziare.

---

## Utilizzo

- **Avvio rapido**: usa `Cmd+Esc` (Mac) o `Ctrl+Esc` (Windows/Linux) per aprire AniCode in una vista terminale divisa, oppure per mettere a fuoco una sessione esistente se e gia in esecuzione.
- **Nuova sessione**: usa `Cmd+Shift+Esc` (Mac) o `Ctrl+Shift+Esc` (Windows/Linux) per avviare una nuova sessione di AniCode, anche se ne esiste gia una aperta. Puoi anche cliccare il pulsante AniCode nell'interfaccia.
- **Consapevolezza del contesto**: condividi automaticamente con AniCode la selezione corrente o la scheda attiva.
...
Per installare AniCode su VS Code e fork popolari come Cursor, Windsurf, VSCodium:
...
Cerca **AniCode** nel Marketplace delle estensioni e clicca **Install**.

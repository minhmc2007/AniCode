---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di AniCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

AniCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire AniCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di AniCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a AniCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da AniCode. _Alias_: `/quit`, `/q`
...
- `leader_timeout` - Controlla per quanto tempo AniCode attende dopo il leader key. Default: `2000`.

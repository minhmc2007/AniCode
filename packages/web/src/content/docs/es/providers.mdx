---
title: Proveedores
description: Usando cualquier proveedor LLM en AniCode.
---

import config from "../../../../config.mjs"
export const console = config.console

AniCode usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración AniCode.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu AniCode
configuración.
...
OpenCode Zen es una lista de modelos proporcionados por el equipo AniCode que han sido
probado y verificado para funcionar bien con AniCode. [Más información](/docs/zen).
...
Funciona como cualquier otro proveedor en AniCode y su uso es completamente opcional.

---

## OpenCode Go

OpenCode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de AniCode que han sido
probado y verificado para funcionar bien con AniCode.
...
Para usar Amazon Bedrock con AniCode:
...
:::info
El uso de su suscripción Claude Pro/Max en AniCode no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
AniCode utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear AniCode
...
Para utilizar Google Vertex AI con AniCode:
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación AniCode como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación AniCode enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para AniCode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con AniCode:
...
3. Copie la clave API para usarla en AniCode.
...
6. **Importante**: Antes de usar modelos en la nube en AniCode, debe obtener la información del modelo localmente:
...
OpenCode Zen es una lista de modelos probados y verificados proporcionada por el equipo AniCode. [Más información](/docs/zen).
...
3. Ingrese su clave AniCode API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con AniCode:
...
Los campos `limit` le permiten a AniCode comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.

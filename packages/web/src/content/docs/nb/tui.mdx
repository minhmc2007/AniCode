...
AniCode gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre AniCode starter TUI for gjeldende katalog.
...
Når du bruker AniCode TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til AniCode. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt AniCode. _Aliaser_: `/quit`, `/q`
...
- `leader_timeout` - Styrer hvor lenge AniCode venter etter leader key. Standard er `2000`.

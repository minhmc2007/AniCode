---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en AniCode.
---

import config from "../../../../config.mjs"
export const console = config.console

AniCode utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration AniCode.
...
Vous pouvez personnaliser les fournisseurs via la section `provider` de votre AniCode
configuration.
...
OpenCode Zen est une liste de modèles fournis par l'équipe AniCode qui ont été
testé et vérifié pour fonctionner correctement avec AniCode. [En savoir plus](/docs/zen).
...
Il fonctionne comme n’importe quel autre fournisseur dans AniCode et son utilisation est totalement facultative.

---

## OpenCode Go

OpenCode Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe AniCode qui ont été
testé et vérifié pour fonctionner correctement avec AniCode.
...
Pour utiliser Amazon Bedrock avec AniCode :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans AniCode n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
AniCode utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller AniCode
...
Pour utiliser Google Vertex AI avec AniCode :
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation AniCode en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation AniCode répertoriée comme une session distincte.
...
:::tip
Ollama peut se configurer automatiquement pour AniCode. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/opencode) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec AniCode :
...
3. Copiez la clé API à utiliser dans AniCode.
...
6. **Important** : Avant d'utiliser des modèles cloud dans AniCode, vous devez extraire les informations du modèle localement :
...
OpenCode Zen est une liste de modèles testés et vérifiés fournie par l'équipe AniCode. [En savoir plus](/docs/zen).
...
3. Entrez votre clé AniCode API.
...
Les champs `limit` permettent à AniCode de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.

---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u AniCode.
---

import config from "../../../../config.mjs"
export const console = config.console

AniCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj AniCode konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem AniCode
config.
...
OpenCode Zen je lista modela koje je obezbedio AniCode tim koji su bili
testirano i potvrđeno da dobro radi sa AniCode. [Saznajte više](/docs/zen).
...
Radi kao i svaki drugi provajder u AniCode i potpuno je opcionalan za korištenje.

---

## OpenCode Go

OpenCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža AniCode tim i koji su testirani i verificirani da dobro rade s AniCode-om.
...
Da biste koristili Amazon Bedrock s AniCode:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u AniCode.
:::
...
AniCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate AniCode
...
Za korištenje Google Vertex AI s AniCode:
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki AniCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki AniCode razgovor naveden kao zasebna sesija.
...
:::tip
Ollama se može automatski konfigurirati za AniCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s AniCode:
...
3. Kopirajte API ključ za korištenje u AniCode.
...
6. **Važno**: Prije upotrebe modela oblaka u AniCode, morate lokalno povući informacije o modelu:
...
OpenCode Zen je lista testiranih i verifikovanih modela koju je obezbedio AniCode tim. [Saznajte više](/docs/zen).
...
3. Unesite svoj AniCode API ključ.
...
Polja `limit` omogućavaju AniCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
2. Za prilagođene provajdere, provjerite AniCode konfiguraciju i:

...
Možete dodati vanjske alate u AniCode koristeći _Model Context Protocol_, ili MCP. AniCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [AniCode Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
AniCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, AniCode će:
...
Ako server zahtijeva autentifikaciju, AniCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, AniCode sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u AniCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz AniCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete AniCode sa vašim Sentry nalogom.

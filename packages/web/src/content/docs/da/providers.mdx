---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i AniCode.
---

import config from "../../../../config.mjs"
export const console = config.console

AniCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i AniCode-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i AniCode-konfigurationen.
...
OpenCode Zen er en liste over modeller leveret af AniCode-teamet, som er blevet testet og verificeret til at fungere godt med AniCode. [Læs mere](/docs/zen).
...
Det fungerer som alle andre udbydere i AniCode og er helt valgfrit at bruge.

---

## OpenCode Go

OpenCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af AniCode-teamet, som er testet og verificeret til at fungere godt med AniCode.
...
Sådan bruger du Amazon Bedrock med AniCode:
...
:::info
At bruge dit Claude Pro/Max-abonnement i AniCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
AniCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse AniCode
...
Sådan bruger du GitHub Copilot-abonnementet med AniCode:
...
Sådan bruger du Google Vertex AI med AniCode:
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver AniCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver AniCode-samtale opført som en separat session.
...
:::tip
Ollama kan automatisk konfigurere sig selv for AniCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med AniCode:
...
3. Kopier API-nøglen til brug i AniCode.
...
6. **Vigtigt**: Før du bruger skymodeller i AniCode, skal du hente modelinformationen lokalt:
...
OpenCode Zen er en liste over testede og verificerede modeller leveret af AniCode-teamet. [Læs mere](/docs/zen).
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med AniCode:
...
`limit`-felterne lader AniCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.

...
import { isRecord } from "@anicode-ai/tui/util/record"
...
import { TuiKeybind } from "@anicode-ai/tui/config/keybind"
import { InstallationLocal, InstallationVersion } from "@anicode-ai/core/installation/version"
import { makeRuntime } from "@anicode-ai/core/effect/runtime"
import { Filesystem } from "@/util/filesystem"
import { ConfigVariable } from "@/config/variable"
import { Npm } from "@anicode-ai/core/npm"
import { FormatError, FormatUnknownError } from "@/cli/error"
import { TuiConfig } from "@anicode-ai/tui/config"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // 4. `.opencode` directories (and ANICODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".opencode") || dir === Flag.ANICODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".opencode") && dir !== Flag.ANICODE_CONFIG_DIR) continue

...
Move the canonical AniCode terminal application from
...
The SDK is the TUI's AniCode boundary. Missing backend data or operations must
...
- AniCode domain data displayed by the TUI
...
route, and feature-plugin code no longer reads AniCode globals or process state;
...
Replace process-global AniCode state with resolved TUI inputs.
...
adapters rather than AniCode domain implementations.

Make the SDK the only AniCode domain boundary used by the TUI.
...
- Domain-facing TUI code imports AniCode data and operations only from
  `@opencode-ai/sdk`.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for AniCode domain state.
